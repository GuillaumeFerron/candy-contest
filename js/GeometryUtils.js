 ...
 */
 ...
 var GeometryUtils = {
	...
	letterN: function (center, size) {
		var center = center !== undefined ? center : new Vector3( 0, 0, 0 ),
		size = size !== undefined ? size : 10,
		half = size / 2

		const centeringV = new Vector3(size / 2, size / 2, size / 2)

		const _v = [
			new Vector3(0, 0, 0).sub(centeringV),
			new Vector3(0, size, 0).sub(centeringV),
			new Vector3(size / 3, size, 0).sub(centeringV),
			new Vector3(2 * size / 3, size / 3, 0).sub(centeringV),
			new Vector3(2 * size / 3, size, 0).sub(centeringV),
			new Vector3(size, size, 0).sub(centeringV),
			new Vector3(size, 0, 0).sub(centeringV),
			new Vector3(2 * size / 3, 0, 0).sub(centeringV),
			new Vector3(size / 3, 2 * size / 3, 0).sub(centeringV),
			new Vector3(size / 3, 0, 0).sub(centeringV),
			new Vector3(0, 0, 0).sub(centeringV),
		]
